...
const { reloadPM2Process, showPM2ProcessInfo } = require('./pm2');
...
exports.executeRule = async function executeRule(rule, skip = false) {
  try {
    switch (rule.type) {
      case 'httpStatus': {
        try {
          ...
          if (statusCode !== rule.options.statusCode) {
            if (skip) return true;
            await reloadPM2Process(rule.serviceName);
          }
        } catch (err) {
          if (skip) return true;
          await reloadPM2Process(rule.serviceName);
      ...
      case 'memoryUsage': {
        ...
        for (const processInfo of processesInfo) {
          if (processInfo.monit.memory > getBytes(rule.options.usage)) {
            if (skip) return true;
            await reloadPM2Process(processInfo.pm_id);
          }
        }
      }
      case 'cpuUsage': {
